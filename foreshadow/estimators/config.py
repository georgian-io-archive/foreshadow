...
def get_tpot_config(type_, include_preprocessors=False):
    dicts = {
        "classification": tpot_classification_config,
        "regression": tpot_regression_config,
    }
    return {
        sub_key: v
        for k in dicts[type_].keys()
        for sub_key, v in dicts[type_][k].items()
        if not k == "preprocessors" or include_preprocessors
    }
