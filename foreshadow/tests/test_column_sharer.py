...
@pytest.mark.parametrize(
    'key,expected_error,expected_str1,expected_str2',
    [
        ('testing', None, None, '.+already a registered key.+'),
        ('domain',
         KeyError,
...
def test_column_sharer_register(key, expected_error, expected_str1,
                                expected_str2):
    ...
    if expected_error is not None:
        with pytest.raises(expected_error) as e1:
            cs.register_key(key)
        with pytest.raises(expected_error) as e2:
            cs.register_key(key)
    else:
        cs.register_key(key)
    if expected_error is not None:
...
def test_column_sharer_register(key, expected_error1, expected_error2,
                                expected_str1, expected_str2):
    """Test registering key, ensuring key is on dict and error is raised.
    ...
    if len(key) == 1:
        ...
        try:  # assume expected is an error
            if issubclass(expected,
                          BaseException):  # this will fail if it isn't
                with pytest.raises(expected) as e:
                    cs[key[0]]
                assert issubclass(e.type, expected)
        except TypeError:  # then expected will be the true result returned
...
def test_column_sharer_iter(store):
    ...
    for key in iter(cs.store):
        expected[key] = cs.store[key]
    assert expected == cs